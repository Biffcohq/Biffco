      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
        {isLoading ? (
          ...
          <Link href={`/w/${workspace?.id || 'default'}`} className="block group">
            <div className="bg-surface border border-border rounded-xl p-6 shadow-sm hover:shadow-md hover:border-primary/50 transition-all cursor-pointer flex flex-col h-40 relative overflow-hidden">
              ...
              <h2 className="text-lg font-bold text-text-primary group-hover:text-primary transition-colors">{workspace?.name || 'Estancia Los Alamos'}</h2>
              <p className="text-sm text-text-secondary mt-1">Tenant ID: {workspace?.id || 'wk_12345'}</p>
            </div>
          </Link>
        )}
